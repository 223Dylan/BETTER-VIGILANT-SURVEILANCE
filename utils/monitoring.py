...
class SystemMonitor:
    ...
    def __init__(self, port: int = 8000):
        ...
        try:
            ...
            self.active_cameras = self._create_or_get_gauge(
                "active_cameras", "Number of active cameras"
            )
            self.camera_fps = self._create_or_get_gauge(
                "camera_fps", "Frames per second per camera", ["camera_id"]
            )
            self.camera_latency = self._create_or_get_histogram(
                "camera_latency", "Processing latency per camera", ["camera_id"]
            )
            self.camera_errors = self._create_or_get_counter(
                "camera_errors", "Error count per camera", ["camera_id", "error_type"]
            )

            # System metrics
            self.cpu_usage = self._create_or_get_gauge(
                "cpu_usage", "CPU usage percentage"
            )
            self.memory_usage = self._create_or_get_gauge(
                "memory_usage", "Memory usage percentage"
            )
            self.gpu_usage = self._create_or_get_gauge(
                "gpu_usage", "GPU usage percentage"
            )
            self.disk_usage = self._create_or_get_gauge(
                "disk_usage", "Disk usage percentage"
            )

            # Model metrics
            self.model_inference_time = self._create_or_get_histogram(
                "model_inference_time", "Model inference time in seconds", ["camera_id"]
            )
            self.model_confidence = self._create_or_get_gauge(
                "model_confidence", "Model confidence score", ["camera_id"]
            )
    ...
    def _create_or_get_gauge(
        self, name: str, description: str, labelnames: list = None
    ):
    ...
    def record_model_metrics(
        self, camera_id: str, inference_time: float, confidence: float
    ):
