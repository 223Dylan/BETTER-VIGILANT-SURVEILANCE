        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                if response.status_code in [200, 201]:
                    ...
                    print(f"[ERROR] Failed to create template {template['name']}: {response.text}")
...
def test_data_flow():
    ...
    try:
        response = requests.get(f"{ELASTICSEARCH_URL}/detection-metrics-*/_search?size=1&sort=@timestamp:desc")
        if response.status_code == 200:
            data = response.json()
            if data['hits']['total']['value'] > 0:
                print("[SUCCESS] Recent detection metrics found")
                latest = data['hits']['hits'][0]['_source']
                print(f"   Latest: {latest.get('type', 'unknown')} from {latest.get('camera_id', 'unknown')}")
            else:
                print("[WARNING] No detection metrics found - start your detection system")
        
        response = requests.get(f"{ELASTICSEARCH_URL}/system-performance-*/_search?size=1&sort=@timestamp:desc")
        if response.status_code == 200:
            data = response.json()
            if data['hits']['total']['value'] > 0:
...
def print_dashboard_instructions():
    ...
    print("   - Metrics: Avg of 'performance.fps_actual' and 'performance.processing_latency_ms'")
...
def main():
    ...
    print("3. Check recent data:", f"{ELASTICSEARCH_URL}/detection-metrics-*/_search?size=1")
