...
def create_index_templates():
    ...
    alerting_config_path = Path(__file__).parent.parent / "kibana" / "detection_metrics_alerting.json"
    ...
    try:
        ...
        for index_pattern in config['index_patterns']:
            ...
            if response.status_code in [200, 201]:
                ...
                print(f"[ERROR] Failed to create index template {template_name}: {response.text}")
...
def import_kibana_dashboards():
    ...
    dashboard_path = Path(__file__).parent.parent / "kibana" / "dashboards" / "detection-system-enhanced.json"
...
def test_logging_pipeline():
    ...
    try:
        ...
        from detection_metrics import log_prediction_metrics, log_system_metrics, log_camera_health_metrics
...
def verify_setup():
    ...
    if response.status_code == 200 and response.json():
        ...
        print("[WARNING] No detection indices found (this is normal if no data has been logged yet)")
