...
async def get_alert_stats(
    ...
    try:
        ...
        return AlertResponse(
            success=True, message=f"Retrieved statistics for {days} days", data=stats
        )
...
async def search_alerts(
    ...
    try:
        ...
        historical_alerts = alert_service.get_alert_history(
            limit * 2, filter_dict
        )  # Get more history for better results
...
async def acknowledge_alert(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=404, detail="Alert not found or cannot be acknowledged"
            )
...
async def resolve_alert(
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=404, detail="Alert not found or cannot be resolved"
            )
    ...
    try:
        ...
        for alert_id in alert_ids:
            success = alert_service.acknowledge_alert(
                alert_id, action.userId, action.notes
            )
        ...
        logger.info(
            f"[SUCCESS] Bulk acknowledged {success_count}/{len(alert_ids)} alerts by {action.userId}"
        )
...
async def dismiss_alert(
    ...
    try:
        ...
        success = alert_service.resolve_alert(
            alert_id,
            action.userId,
            f"DISMISSED: {action.notes or 'No reason provided'}",
        )
        ...
        if success:
            ...
            raise HTTPException(
                status_code=404, detail="Alert not found or cannot be dismissed"
            )
...
async def create_sample_alerts(
    ...
    try:
        ...
        logger.info(
            f"[SUCCESS] Created {len(created_alerts)} sample alerts for testing"
        )

        return AlertResponse(
            success=True,
            message=f"Created {len(created_alerts)} sample alerts",
            data={"created_alert_ids": created_alerts, "total": len(created_alerts)},
        )
...
async def manual_auto_clear_alerts(
    ...
    try:
        logger.info(
            f"[AUTO-CLEAR] Manual auto-clearance triggered by user {current_user.username}"
        )

        cleared_count = alert_service.auto_clear_old_alerts()

        logger.info(
            f"[SUCCESS] Manual auto-clearance completed: {cleared_count} alerts cleared"
        )
...
async def get_auto_clear_config(
    ...
    try:
        ...
        config = {
            ...
            "critical_timeout_hours": int(
                os.getenv("ALERT_CRITICAL_AUTO_CLEAR_HOURS", "24")
            ),
            "interval_minutes": int(
                os.getenv("ALERT_AUTO_CLEAR_INTERVAL_MINUTES", "60")
            ),
        }
        ...
        return AlertResponse(
            success=True, message="Auto-clearance configuration retrieved", data=config
        )
...
async def bulk_alert_action(
    ...
    try:
        ...
        if request.action not in ["acknowledge", "resolve"]:
            raise HTTPException(
                status_code=400, detail="Action must be 'acknowledge' or 'resolve'"
            )

        logger.info(
            f"[BULK] User {current_user.username} performing {request.action} on {len(request.alert_ids)} alerts"
        )
        ...
        for alert_id in request.alert_ids:
            try:
                ...
                if success:
                    ...
                    logger.debug(
                        f"[BULK] Successfully {request.action}d alert {alert_id}"
                    )
                else:
                    ...
                    logger.warning(
                        f"[BULK] Failed to {request.action} alert {alert_id} - not found"
                    )
        ...
        success_rate = (
            len(successful_actions) / total_processed if total_processed > 0 else 0
        )

        logger.info(
            f"[BULK] Completed bulk {request.action}: {len(successful_actions)} successful, {len(failed_actions)} failed"
        )
