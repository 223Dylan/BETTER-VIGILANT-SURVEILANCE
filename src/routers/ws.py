...
async def prediction_ws(websocket: WebSocket, camera_id: str):
    ...
    logger.info(f"[WEBSOCKET] Prediction WebSocket connection attempt for camera {camera_id}")
    ...
    try:
        ...
        logger.info(f"[WEBSOCKET] Prediction WebSocket registered in manager for camera {camera_id}")
        ...
        while True:
            try:
                ...
                logger.debug(f"[RECEIVED] Received client message for {camera_id}: {data}")
                ...
                logger.debug(f"[KEEPALIVE] Sent keepalive to {camera_id}, connections: {connection_count}")
                
    except WebSocketDisconnect:
        logger.info(f"[DISCONNECTED] Prediction WebSocket disconnected for camera {camera_id}")
        await websocket_manager.disconnect_prediction(camera_id, websocket)
    except Exception as e:
        logger.error(f"[ERROR] Error in prediction WebSocket for camera {camera_id}: {str(e)}")
        await websocket_manager.disconnect_prediction(camera_id, websocket)
        try:
            await websocket.send_json({
                "type": "error",
