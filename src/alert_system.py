...
class AlertSystem:
    def __init__(self):
        ...
        logger.info(f"Alert system initialized with available channels: {', '.join(self.available_channels)}")
    ...
    def _initialize_channels(self) -> List[str]:
        ...
        if os.path.exists('config/email_config.json'):
            try:
                ...
                if all(k in self.email_config for k in ['smtp_server', 'smtp_port', 'username', 'password']):
    ...
    def send_alert(self, message: str, level: str = 'info', channels: Optional[List[str]] = None) -> None:
