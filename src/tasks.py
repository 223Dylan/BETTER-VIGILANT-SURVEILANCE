...
def predict_sequence(self, sequence_data):
    ...
    try:
        ...
        logger.info(f"Processing sequence of shape {sequence.shape} for camera {camera_id}")
        ...
        logger.info(f"[STATS] WORKER RECEIVED SEQUENCE: mean={seq_mean:.4f}, std={seq_std:.4f}, min={seq_min:.4f}, max={seq_max:.4f}")
        
        # Compare first and last frames
        if sequence.shape[0] >= 2:
            first_frame_mean = sequence[0].mean()
            last_frame_mean = sequence[-1].mean()
            logger.info(f"[ANALYSIS] FRAME VARIATION: first_frame_mean={first_frame_mean:.4f}, last_frame_mean={last_frame_mean:.4f}")
            
        # Data quality checks
        if seq_std < 0.001:
            logger.warning("[WARNING] WORKER WARNING: Sequence appears to have no variation (std < 0.001)")
        elif np.allclose(sequence[0], sequence[-1]):
            logger.warning("[WARNING] WORKER WARNING: First and last frames are identical - possible repeated data")
        ...
        if camera_id and camera_id != 'unknown':
            try:
                ...
                logger.info(f"[SUCCESS] Logged detection metrics for camera {camera_id}")
            except Exception as e:
                logger.error(f"[ERROR] Failed to log detection metrics for {camera_id}: {e}")
        ...
        if camera_id and camera_id != 'unknown':
            try:
                ...
                logger.info(f"[SUCCESS] Successfully routed prediction to WebSocket for camera {camera_id}")
            except Exception as e:
                logger.error(f"[ERROR] Failed to route prediction to WebSocket for {camera_id}: {e}")
        else:
            logger.warning(f"[WARNING] No camera_id provided, skipping WebSocket routing")
