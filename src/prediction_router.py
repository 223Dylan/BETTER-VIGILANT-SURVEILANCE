    ...
    try:
        ...
        if _websocket_loop and not _websocket_loop.is_closed():
            future = asyncio.run_coroutine_threadsafe(
                send_prediction_to_websocket(camera_id, alert_data), _websocket_loop
            )
            # Don't wait for result to avoid blocking Celery worker
            logger.info(
                f"[SCHEDULED] Scheduled prediction broadcast for camera {camera_id}"
            )
        else:
        ...
        logger.error(
            f"[ERROR] Error routing prediction to WebSocket for {camera_id}: {e}"
        )
...
def format_prediction_for_alert(prediction_result: Dict[Any, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        f"[ALERT] Formatted alert: {alert_type} (confidence: {confidence:.2f}) for camera {prediction_result.get('camera_id')}"
    )
    return alert_data
