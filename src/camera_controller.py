...
class CameraController:
    ...
    def start_camera(self, camera_id: str) -> bool:
        ...
        try:
            ...
            if process.is_alive():
                ...
                camera_db_service.update_camera_status(
                    camera_id, "error", "Failed to start process"
                )
                self._invalidate_cache()  # Invalidate cache on status change
    ...
    def stop_camera(self, camera_id: str) -> bool:
        ...
        try:
            ...
            if process.is_alive():
                logger.warning(
                    f"Process for camera '{camera_id}' did not terminate gracefully. Forcing termination."
                )
                process.terminate()
                process.join()
    ...
    def get_all_camera_status(self) -> Dict[str, str]:
        ...
        ) < self._cache_duration and self._status_cache:
    ...
    def update_camera_config(self, camera_id: str, updates: Dict) -> bool:
        ...
        try:
            ...
            is_running = (
                camera_id in self.processes and self.processes[camera_id].is_alive()
            )
            ...
            if success and is_running:
                ...
                if "brightness" in updates:
                    ...
                    logger.info(
                        f"Sent dynamic brightness update: {updates['brightness']} for camera '{camera_id}'"
                    )
                ...
                if restart_needed:
                    logger.info(
                        f"Restarting camera '{camera_id}' due to configuration change requiring restart"
                    )
