...
class CameraConfig:
    ...
    def url(self) -> str:
        ...
        if self.camera_type == "usb":
            ...
            auth = f"{self.credentials['username']}:{self.credentials['password']}@" if self.credentials else ""
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'CameraConfig':
        ...
        try:
            ...
            required_fields = ['id', 'name', 'location_id', 'zone_name', 'source', 'camera_type']
...
def load_camera_configs(config_path: str = "config/config.yaml") -> Dict[str, CameraConfig]:
    ...
    try:
        if os.path.exists(config_path):
            ...
            if isinstance(cameras_data, list):
                return {config['id']: CameraConfig.from_dict(config) for config in cameras_data}
            else:
                return {cam_id: CameraConfig.from_dict(config) 
