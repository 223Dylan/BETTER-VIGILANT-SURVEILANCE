...
class SecretsManager:
    ...
    def validate_secrets(self) -> bool:
        ...
        missing_required = [
            secret for secret in required_secrets if not self._secrets.get(secret)
        ]
        if missing_required:
        ...
        missing_optional = [
            secret for secret in optional_secrets if not self._secrets.get(secret)
        ]
        if missing_optional:
            logger.info(
                f"Optional secrets not configured: {', '.join(missing_optional)}"
            )
