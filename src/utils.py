...
def validate_global_config(config: dict):
    required_sections = [
        'model', 'processing', 'logging', 'preprocessing'
    ]
...
def validate_camera_config(cam: dict):
    required_fields = [
        'id', 'name', 'location_id', 'zone_name', 'source', 'camera_type', 'fps'
    ]
    ...
    if not isinstance(cam['fps'], int) or cam['fps'] <= 0:
        raise ValueError(f"'fps' must be a positive integer in camera config: {cam['id']}")
    if not isinstance(cam['resolution_width'], int) or cam['resolution_width'] <= 0:
        raise ValueError(f"'resolution_width' must be a positive integer in camera config: {cam['id']}")
    if not isinstance(cam['resolution_height'], int) or cam['resolution_height'] <= 0:
        raise ValueError(f"'resolution_height' must be a positive integer in camera config: {cam['id']}")
    if 'logging_level' in cam and cam['logging_level'] not in ('DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL'):
        raise ValueError(f"Invalid logging_level: {cam['logging_level']}. Must be one of: DEBUG, INFO, WARNING, ERROR, CRITICAL")
    # Validate alert configuration
    if 'alerts' in cam:
        alerts = cam['alerts']
        if not isinstance(alerts, dict):
            raise ValueError(f"'alerts' must be a dictionary in camera config: {cam['id']}")
        # Validate webhooks
        if 'webhooks' in alerts:
            webhooks = alerts['webhooks']
            if not isinstance(webhooks, list):
                raise ValueError(f"'alerts.webhooks' must be a list in camera config: {cam['id']}")
            for webhook in webhooks:
                if not isinstance(webhook, dict):
                    raise ValueError(f"Each webhook must be a dictionary in camera config: {cam['id']}")
                if 'url' not in webhook:
                    raise ValueError(f"Webhook missing required 'url' field in camera config: {cam['id']}")
        # Validate email
        if 'email' in alerts:
            email = alerts['email']
            if not isinstance(email, dict):
                raise ValueError(f"'alerts.email' must be a dictionary in camera config: {cam['id']}")
            if 'recipients' not in email:
                raise ValueError(f"'alerts.email' missing required 'recipients' field in camera config: {cam['id']}")
            if not isinstance(email['recipients'], list):
                raise ValueError(f"'alerts.email.recipients' must be a list in camera config: {cam['id']}")
        # Validate cooldown
        if 'cooldown' in alerts:
            cooldown = alerts['cooldown']
            if not isinstance(cooldown, (int, float)) or cooldown < 0:
                raise ValueError(f"'alerts.cooldown' must be a positive number in camera config: {cam['id']}")
    # Validate ROIs
    if 'rois' in cam:
        rois = cam['rois']
        ...
        for roi in rois:
            if not isinstance(roi, dict):
                raise ValueError(f"Each ROI must be a dictionary in camera config: {cam['id']}")
            required_roi_fields = ['name', 'points']
            for field in required_roi_fields:
                if field not in roi:
                    raise ValueError(f"ROI missing required field '{field}' in camera config: {cam['id']}")
            if not isinstance(roi['points'], list) or len(roi['points']) < 3:
                raise ValueError(f"ROI 'points' must be a list of at least 3 points in camera config: {cam['id']}")
            for point in roi['points']:
                if not isinstance(point, list) or len(point) != 2:
                    raise ValueError(f"ROI point must be a list of 2 coordinates [x,y] in camera config: {cam['id']}")
                if not all(isinstance(coord, (int, float)) for coord in point):
                    raise ValueError(f"ROI coordinates must be numbers in camera config: {cam['id']}")
    # Validate model parameters
    if 'model_params' in cam:
        model_params = cam['model_params']
        if not isinstance(model_params, dict):
            raise ValueError(f"'model_params' must be a dictionary in camera config: {cam['id']}")
        # Validate sequence_length
        if 'sequence_length' in model_params:
            seq_len = model_params['sequence_length']
            if not isinstance(seq_len, int) or seq_len <= 0:
                raise ValueError(f"'model_params.sequence_length' must be a positive integer in camera config: {cam['id']}")
        # Validate frame_size
        if 'frame_size' in model_params:
            ...
            if not isinstance(frame_size, (list, tuple)) or len(frame_size) != 2:
                raise ValueError(f"'model_params.frame_size' must be a list/tuple of 2 integers in camera config: {cam['id']}")
            if not all(isinstance(dim, int) and dim > 0 for dim in frame_size):
                raise ValueError(f"'model_params.frame_size' dimensions must be positive integers in camera config: {cam['id']}")
        # Validate batch_size
        if 'batch_size' in model_params:
            batch_size = model_params['batch_size']
            if not isinstance(batch_size, int) or batch_size <= 0:
                raise ValueError(f"'model_params.batch_size' must be a positive integer in camera config: {cam['id']}")
        ...
        if 'inference_mode' in model_params:
            ...
            if inference_mode not in ('cpu', 'gpu', 'tpu'):
                raise ValueError(f"'model_params.inference_mode' must be one of: cpu, gpu, tpu in camera config: {cam['id']}")
