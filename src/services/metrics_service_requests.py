...
class MetricsServiceRequests:
    """Alternative metrics service using requests library instead of aiohttp."""
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning(f"Failed to initialize Prometheus monitor: {e}")
            self.prometheus_monitor = None
    
    def _query_elasticsearch_sync(self, index: str, query: dict) -> dict:
        """Execute synchronous query against Elasticsearch."""
        try:
    ...
    async def _query_elasticsearch(self, index: str, query: dict) -> dict:
        """Execute async query against Elasticsearch using thread pool."""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            self.executor, 
            self._query_elasticsearch_sync, 
            index, 
            query
        )
    ...
    async def get_system_metrics(self, time_range: str = "15m", limit: int = 100) -> List[dict]:
        ...
        query = {
            ...
            "_source": ["@timestamp", "cpu_usage", "memory_usage", "disk_usage", "active_cameras"]
        }
    ...
    async def get_camera_metrics(self) -> List[dict]:
                    ...
                    "aggs": {
                        "latest_metrics": {
                            "top_hits": {
                                ...
                                "size": 1,
                                "_source": ["fps_actual", "fps_target", "latency_ms", "status", "last_detection"]
                            }
                        }
                    }
                }
        ...
        cameras = []
        for bucket in result.get("aggregations", {}).get("cameras", {}).get("buckets", []):
    ...
    async def get_camera_performance(self, camera_id: str, time_range: str = "1h") -> dict:
        ...
        query = {
            ...
            "_source": ["@timestamp", "fps_actual", "latency_ms", "queue_depth", "dropped_frames"]
        }
    ...
    async def get_detection_metrics(self, time_range: str = "1h", camera_id: Optional[str] = None, confidence_threshold: float = 0.0) -> List[dict]:
        ...
        query = {
            ...
            "_source": ["@timestamp", "camera_id", "prediction.confidence", "prediction.label", 
                       "prediction.is_shoplifting", "alert.triggered"]
        }
    ...
    async def get_metrics_summary(self) -> dict:
        ...
        latest_system = system_metrics[0] if system_metrics else {
            "timestamp": datetime.now().isoformat(),
            "cpu_usage": 0, "memory_usage": 0, "disk_usage": 0, "active_cameras": 0
        }
        ...
        recent_detections = await self.get_detection_metrics(time_range="1h", confidence_threshold=0.5)
    ...
    def _check_elasticsearch_health_sync(self) -> bool:
        """Synchronously check Elasticsearch health."""
        try:
    ...
    async def get_health_status(self) -> dict:
        ...
        # Check Elasticsearch async
        loop = asyncio.get_event_loop()
        health["elasticsearch"] = await loop.run_in_executor(
            self.executor, 
            self._check_elasticsearch_health_sync
        )
    ...
    async def get_recent_alerts(self, limit: int = 50, severity: Optional[str] = None) -> List[dict]:
        ...
        query = {
            ...
            "_source": ["@timestamp", "camera_id", "alert.level", "alert.type", 
