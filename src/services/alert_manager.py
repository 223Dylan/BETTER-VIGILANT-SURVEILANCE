...
class AlertManager:
    ...
    def process_prediction(self, prediction_result: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            timestamp = prediction_result.get('timestamp', datetime.utcnow().timestamp())
            ...
            if not self._should_create_alert(alert_type, severity, confidence):
                logger.debug(f"Skipping alert creation for {camera_id}: {alert_type.value} (confidence: {confidence:.3f})")
                return None
            ...
            try:
                ...
                logger.error(f"[ERROR] Failed to save alert {alert_id} to database: {e}")
            
            logger.info(f"[ALERT] Created {severity.value.upper()} alert: {alert_type.value} for {camera_id} (confidence: {confidence:.3f})")
    ...
    def _determine_alert_type(self, confidence: float, is_shoplifting: bool) -> AlertType:
    ...
    def _determine_severity(self, confidence: float, is_shoplifting: bool) -> AlertSeverity:
    ...
    def _should_create_alert(self, alert_type: AlertType, severity: AlertSeverity, confidence: float) -> bool:
    ...
    def _generate_alert_message(self, alert_type: AlertType, confidence: float, camera_id: str) -> str:
        ...
        if alert_type == AlertType.SHOPLIFTING:
            if confidence >= 0.9:
                ...
                return f"[WARNING] SHOPLIFTING DETECTED on {camera_id} ({confidence_pct}%)"
            else:
    ...
    async def _broadcast_alert(self, alert: AlertRecord):
        ...
        try:
            ...
            logger.info(f"[BROADCAST] Broadcasted alert {alert.id} for camera {alert.camera_id}")
    ...
    def resolve_alert(self, alert_id: str, user_id: str, notes: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.error(f"[ERROR] Failed to update resolved alert {alert_id} in database: {e}")
    ...
    def get_active_alerts(self, filters: Optional[Dict] = None) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"[ERROR] Error getting alerts from database, falling back to memory: {e}")
    ...
    def get_alert_history(self, limit: int = 100, filters: Optional[Dict] = None) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"[ERROR] Error getting alert history from database, falling back to memory: {e}")
    ...
    def get_alert_stats(self, days: int = 7) -> Dict:
        ...
        try:
            ...
            logger.error(f"[ERROR] Error getting alert stats from database, falling back to memory: {e}")
    ...
    def _apply_filters(self, alerts: List[AlertRecord], filters: Dict) -> List[AlertRecord]:
