...
class RedisWebSocketBridge:
    ...
    def publish_websocket_event(self, camera_id: str, event_type: str, data: Dict[str, Any]) -> bool:
    ...
    async def start_subscriber(self, websocket_manager):
        ...
        try:
            ...
            logger.info(f"[REDIS] Started WebSocket event subscriber on channel: {self.channel_name}")
                ...
                try:
                    ...
                    if message and message['type'] == 'message':
                        try:
                            ...
                            logger.info(f"[REDIS] Received {event_type} event for camera {camera_id}")
                            
                            # Broadcast to WebSockets based on event type
                            if event_type == "prediction":
                                await self.websocket_manager.broadcast_prediction(camera_id, data)
                            elif event_type == "alert":
                                await self.websocket_manager.broadcast_alert(camera_id, data)
                            else:
