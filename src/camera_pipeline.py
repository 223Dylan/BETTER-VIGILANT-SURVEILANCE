...
class CameraPipelineProcess(multiprocessing.Process):
    ...
    def run(self):
        ...
        try:
            ...
            if hasattr(self.camera_config, 'preprocessing') and self.camera_config.preprocessing:
                if 'brightness' in self.camera_config.preprocessing:
                    preprocessing_config['brightness'] = self.camera_config.preprocessing['brightness']
            ...
            logger.info(f"Using global config for camera {self.camera_config.id}: preprocessing={model_params['preprocessing']}")
                
            self.frame_processor = FrameProcessor(model_params)
            if not self.frame_processor.initialize():
                logger.error(f"Failed to initialize frame processor for {self.camera_config.id}")
                return
            ...
            logger.error(f"Fatal error in camera pipeline for {self.camera_config.id}: {e}", exc_info=True)
        finally:
            self.cleanup()
    ...
    def _check_and_trigger_predictions(self):
        ...
        try:
            ...
            if sequence is None or len(sequence) == 0:
                logger.error(f"[ERROR] Failed to get sequence for {self.camera_config.id}")
                return
                
            arr = np.array(sequence)
            logger.info(f"[SEQUENCE] SEQUENCE STATS for {self.camera_config.id}: shape={arr.shape}, mean={arr.mean():.4f}, std={arr.std():.4f}")
            
            if arr.std() < 0.001:
                logger.warning(f"[WARNING] SEQUENCE APPEARS TO BE EMPTY/ZEROS for {self.camera_config.id}")
            
            logger.info(f"[PREDICTION] TRIGGERING PREDICTION for {self.camera_config.id}")
            ...
            logger.info(f"[TASK] PREDICTION TASK SENT to Celery: {task.id} for camera {self.camera_config.id}")
            ...
            logger.error(f"[ERROR] ERROR triggering prediction for {self.camera_config.id}: {e}")
    ...
    def _check_brightness_updates(self):
        ...
        try:
            ...
            if command_key in self.shared_data:
                new_brightness = self.shared_data.pop(command_key)  # Remove after reading
                if self.frame_processor:
                    success = self.frame_processor.update_brightness(new_brightness)
                    if success:
                        logger.info(f"Applied brightness update: {new_brightness} for {self.camera_config.id}")
                    else:
                        logger.error(f"Failed to apply brightness update for {self.camera_config.id}")
        except Exception as e:
            logger.error(f"Error checking brightness updates for {self.camera_config.id}: {e}")
