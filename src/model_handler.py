...
class ModelHandler(BaseComponent):
    ...
    def __init__(self, config=None):
        ...
        self.model_path: str = self.get_config('model.path', 'model we use/lrcn_160S_90_90Q.h5')
        self.input_shape: Tuple[int, int, int, int] = tuple(self.get_config('model.input_shape', [160, 90, 90, 1]))
        self.probability_thresholds: Dict[str, int] = self.get_config('processing.probability_thresholds', {
            'low': 75,
            'medium': 85,
            'high': 90
        })
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            dummy_input = np.zeros((self._batch_size, *self.input_shape), dtype=np.float32)
    ...
    def _process_single(self, sequence: np.ndarray) -> Tuple[float, int, str]:
        ...
        try:
            ...
            if sequence.shape != self.input_shape:
                raise ValueError(f"Invalid sequence shape. Expected {self.input_shape}, got {sequence.shape}")
    ...
    def get_prediction_stats(self) -> Dict:
        ...
        try:
            ...
            avg_time = sum(self._prediction_times) / len(self._prediction_times) if self._prediction_times else 0
            # Convert to milliseconds, cap at 10s
            avg_time_ms = min(avg_time * 1000, 10000)
            
            return {
                "avg_prediction_time": avg_time_ms / 1000,  # Convert back to seconds for consistency
