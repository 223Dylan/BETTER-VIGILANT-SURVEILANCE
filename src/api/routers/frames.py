...
async def store_frame(frame_create: FrameCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        frame_record = frame_service.store_frame(
            frame, frame_create.sequence_number, frame_create.metadata
        )
...
async def get_frames_by_sequence(start: int, end: int, db: Session = Depends(get_db)):
...
async def update_frame_metadata(
    frame_id: int, metadata_update: FrameMetadataUpdate, db: Session = Depends(get_db)
):
...
async def delete_old_frames(days: int, db: Session = Depends(get_db)):
