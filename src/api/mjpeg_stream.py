        ...
        try:
            ...
            if camera_controller:
                ...
                if statuses.get(camera_id) != "active":
                    ...
                    if frame_count % 50 == 0:
                        logger.debug(f"Camera {camera_id} is not active, ending stream (frame {frame_count})")
...
def encode_frame_to_jpeg(frame) -> bytes:
    ...
    try:
        ret, jpeg = cv2.imencode('.jpg', np.array(frame), [cv2.IMWRITE_JPEG_QUALITY, 85])
...
def create_test_frame(camera_id: str, message: str = "MJPEG Test Stream"):
    ...
    cv2.putText(frame, f"Camera: {camera_id}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    cv2.putText(frame, timestamp, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    cv2.putText(frame, message, (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
