...
def start_hls_stream(camera_id: str, input_source: str) -> bool:
    ...
    try:
        ...
        ffmpeg_cmd = [
            ...
            "-f",
            "rawvideo",  # Input format (we'll pipe raw frames)
            "-pixel_format",
            "bgr24",
            "-video_size",
            "640x480",  # Adjust based on camera resolution
            "-framerate",
            "15",
            "-i",
            ...
            "libx264",  # Video codec
            "-preset",
            "veryfast",  # Faster than ultrafast but more stable
            "-tune",
            "zerolatency",  # Low latency
            "-g",
            "15",  # GOP size (1 second at 15fps)
            "-keyint_min",
            ...
            "hls",  # Output format
            "-hls_time",
            "3",  # Segment duration (3 seconds for stability)
            "-hls_list_size",
            "3",  # Keep 3 segments in playlist
            "-hls_flags",
            "delete_segments+round_durations",  # Delete old segments and round durations
            "-hls_segment_filename",
            segment_pattern,
            "-hls_playlist_type",
            "event",  # Event playlist type
            "-hls_base_url",
            f"/api/video/hls/{camera_id}/",  # Base URL for segments
            playlist_path,
        ]
        ...
        logger.info(
            f"Started HLS stream for camera {camera_id} with FFmpeg PID {process.pid}"
        )
    ...
    try:
        ...
        if camera_id in hls_output_dirs:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to clean up HLS directory for camera {camera_id}: {e}"
                )
...
def frame_feeder_thread(camera_id: str):
    ...
    while not stop_event.is_set():
        try:
            ...
            if camera_id not in hls_processes:
                logger.warning(
                    f"No HLS process found for camera {camera_id}, stopping feeder"
                )
                break
            ...
            if frame_data is not None:
                ...
                if success:
                    ...
                    if current_time - last_log_time >= 10:  # Every 10 seconds
                        logger.info(
                            f"HLS feeder for {camera_id}: fed {frames_fed} frames"
                        )
                        last_log_time = current_time
                else:
                    logger.warning(
                        f"Failed to feed frame to HLS for camera {camera_id}"
                    )
                    break
            else:
    ...
    logger.info(
        f"Frame feeder thread stopped for HLS camera {camera_id} after feeding {frames_fed} frames"
    )
...
def auto_start_hls_stream(camera_id: str) -> bool:
    ...
    try:
        ...
        feed_thread = threading.Thread(
            target=frame_feeder_thread, args=(camera_id,), daemon=True
        )
...
async def get_hls_playlist(camera_id: str):
    ...
    try:
        ...
        if camera_id not in hls_output_dirs:
            ...
            if not success:
                raise HTTPException(
                    status_code=500, detail="Failed to start HLS stream"
                )
        ...
        if not os.path.exists(playlist_path):
            raise HTTPException(
                status_code=404, detail="Playlist not ready yet, please try again"
            )
        ...
        # Log for debugging
        logger.debug(
            f"Serving HLS playlist for camera {camera_id}, content length: {len(content)}"
        )
...
async def get_hls_segment(camera_id: str, segment_name: str):
    ...
    try:
        ...
        logger.error(
            f"Error serving HLS segment {segment_name} for camera {camera_id}: {e}"
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def start_camera_hls(camera_id: str):
    ...
    try:
        ...
        if success:
            return {
                "message": f"HLS stream started for camera {camera_id}",
                "output_dir": hls_output_dirs.get(camera_id),
            }
        else:
...
async def get_hls_status(camera_id: str):
    ...
    try:
        is_running = (
            camera_id in hls_processes and hls_processes[camera_id].poll() is None
        )

        # Check data availability
        shared_frames_available = (
            shared_frames
            and camera_id in shared_frames
            and shared_frames[camera_id] is not None
        )
        stream_manager_available = stream_manager and hasattr(
            stream_manager, "get_latest_frame"
        )
        stream_manager_frame = None
        if stream_manager_available:
            stream_manager_frame = (
                stream_manager.get_latest_frame(camera_id) is not None
            )
