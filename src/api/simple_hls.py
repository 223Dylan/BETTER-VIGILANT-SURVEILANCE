...
def hls_generator_thread(camera_id: str):
    ...
    frames_per_segment = 10 * segment_duration  # 10 FPS * 2 seconds = 20 frames (lighter)
    segment_index = 0
    frame_count = 0
    frames_for_segment = []
    ...
    while not stop_event.is_set():
        try:
            ...
            if not camera_is_active:
                ...
                if frame_count > 30:  # After ~3 seconds of inactivity (reduced from 10s)
                    logger.info(f"Camera {camera_id} inactive, stopping simple HLS stream")
                    break
            ...
            if frame_data is None:
                ...
                cv2.putText(frame_data, f"HLS Test {timestamp}", (50, 240), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            ...
            if frame_data is not None:
                ...
                if len(frames_for_segment) >= frames_per_segment:
                    create_hls_segment(camera_id, segment_index, frames_for_segment, output_dir)
                    segment_index += 1
                    frames_for_segment = []
...
def create_hls_segment(camera_id: str, segment_index: int, frames: list, output_dir: str):
    ...
    try:
        ...
        ffmpeg_cmd = [
            ...
            "-crf", "28", "-g", "10",  # Lower quality for speed, smaller GOP
        ...
        if result.returncode == 0:
            ...
            if segment_index % 10 == 0:
                logger.debug(f"Created HLS segment {segment_index} for camera {camera_id}")
        else:
...
def create_hls_playlist(camera_id: str, latest_segment: int, output_dir: str):
    ...
    try:
        ...
        if latest_segment % 10 == 0:
            logger.debug(f"Updated HLS playlist for camera {camera_id} with {latest_segment} segments")
...
async def get_simple_hls_segment(camera_id: str, segment_name: str):
    ...
    try:
        ...
        logger.error(f"Error serving simple HLS segment {segment_name} for camera {camera_id}: {e}")
        raise HTTPException(status_code=500, detail=str(e)) 