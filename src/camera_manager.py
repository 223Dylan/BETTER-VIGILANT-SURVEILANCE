...
class CameraManager:
    ...
    def __init__(self):
        ...
        self._running = False
        self._frame_times: Dict[str, deque] = {}  # Track frame times for FPS calculation
        ...
        self._failed_cameras: Dict[str, str] = {}  # Track failed cameras and their error messages
                ...
                for attempt in range(self._init_retries):
                    try:
                        ...
                        if camera.initialize():
                            ...
                            self._frame_times[config.id] = deque(maxlen=self._fps_window)
                            self._last_error_time[config.id] = 0
                            logger.info(f"Initialized camera: {config.name} ({config.id})")
                            success = True
                            break
                        else:
                        ...
                        logger.warning(f"{error_msg} (attempt {attempt + 1}/{self._init_retries})")
                        if attempt < self._init_retries - 1:
                            time.sleep(self._init_retry_delay)
                else:
                    # All retries failed
                    self._failed_cameras[config.id] = error_msg
                    logger.error(f"All initialization attempts failed for camera: {config.name}")
    ...
    def get_frame(self, camera_id: str) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if frame is not None and frame is not prev_last_frame:
                ...
                self._last_error_time[camera_id] = 0  # Reset error time on successful frame
            return frame
            
        except Exception as e:
            current_time = time.time()
            if current_time - self._last_error_time.get(camera_id, 0) > self._error_cooldown:
    ...
    def get_all_frames(self) -> Dict[str, Optional[np.ndarray]]:
        ...
        return {camera_id: self.get_frame(camera_id) 
    ...
    def get_all_status(self) -> Dict[str, dict]:
        ...
        status.update({camera_id: self.get_camera_status(camera_id)
