...
class VideoStreamManager:
    ...
    def process_frame(self, camera_id: str, frame: bytes) -> bool:
        ...
        try:
            ...
            if elapsed >= 10.0:  # Changed from 5 to 10 seconds to reduce logging
                ...
                logger.debug(f"Camera {camera_id} - FPS: {self._fps[camera_id]:.1f}, Buffer: {buffer_size}/{self.buffer_size}")  # Changed to debug level
                self._frames_processed[camera_id] = 0
    ...
    async def start_stream(self, camera_id: str, websocket: WebSocket) -> None:
        ...
        if not self._running or not self._initialized:
            logger.error(f"Stream manager not running or initialized for camera {camera_id}")
        ...
        try:
            ...
            if camera_id not in self.streams:
                logger.warning(f"No frame buffer found for camera {camera_id}, initializing...")
                self.streams[camera_id] = deque(maxlen=self.buffer_size)
            ...
            logger.info(f"[SUCCESS] WebSocket connection FORCE REGISTERED for camera {camera_id}")
            logger.info(f"[SUCCESS] Connection status: {self._connection_status[camera_id]}")
            logger.info(f"[SUCCESS] Active connections: {len(self.connections)}")
            
            # Log buffer status
            buffer_size = len(self.streams.get(camera_id, []))
            logger.info(f"[SUCCESS] Buffer status for {camera_id}: {buffer_size}/{self.buffer_size} frames ready")
                
            logger.info(f"[STREAMING] Started streaming for camera {camera_id} via {ws_url}")
        ...
        try:
            if camera_id in self.connections:
                ...
                if camera_id in self._connection_status:
                    ...
                    logger.info(f"Connection stats for camera {camera_id} - Client: {client_info}:")
                ...
                del self.connections[camera_id]
                del self._connection_status[camera_id]
                logger.info(f"Removed WebSocket connection for camera {camera_id} - Client: {client_info}")
    ...
    async def send_frame(self, camera_id: str) -> None:
        ...
        try:
            ...
            if camera_id not in self.connections and camera_id not in self._connection_status:
                return
                
            # Check connection status first
            if camera_id in self._connection_status and not self._connection_status[camera_id]['connected']:
                return
            ...
            try:
                ...
                if self._frames_sent[camera_id] % 50 == 1:  # Log first frame and every 50th
                    buffer_size = len(self.streams[camera_id])
                    logger.info(f"[SUCCESS] Sent frame #{self._frames_sent[camera_id]} to {camera_id}, Buffer: {buffer_size}/{self.buffer_size}")
                ...
                if camera_id in self._connection_status:
                    self._connection_status[camera_id]['errors'] = self._connection_status[camera_id].get('errors', 0) + 1
    ...
    async def send_error(self, camera_id: str, error_message: str) -> None:
        ...
        if not self._running or not self._initialized:
            logger.warning(f"Stream manager not running or initialized for camera {camera_id}")
            return
    ...
    def get_connection_status(self, camera_id: str) -> dict:
        ...
        if camera_id not in self._connection_status:
            return {
                'connected': False,
                'error': 'No connection status available'
            }
        ...
        if status['connected']:
            ...
            if status['last_frame_time']:
                status['time_since_last_frame'] = time.time() - status['last_frame_time']
            status['buffer_size'] = len(self.streams.get(camera_id, []))
            status['fps'] = status['frames_sent'] / status['duration'] if status['duration'] > 0 else 0
    ...
    def get_buffer_status(self, camera_id: str) -> dict:
        ...
        if camera_id not in self.streams:
            return {"buffer_size": 0, "max_size": self.buffer_size, "status": "not_initialized"}
    ...
    def fix_connection_tracking(self, camera_id: str) -> dict:
        ...
        try:
            ...
            if camera_id in self.streams and len(self.streams[camera_id]) > 0:
                ...
                if camera_id not in self._connection_status:
                    logger.warning(f"Camera {camera_id} has {buffer_size} frames but no connection tracking!")
