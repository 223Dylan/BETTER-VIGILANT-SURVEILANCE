...
class FrameProcessor(BaseComponent):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.sequence_buffer = np.zeros((self.sequence_length, self.frame_size, self.frame_size, 1), dtype=np.float32)
        ...
        self.frame_queue = deque(maxlen=10)  # Increased from 5 to 10 to prevent frame drops
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"FrameProcessor initialized with sequence length {self.sequence_length} and frame size {self.frame_size}")
            return True
    ...
    def _process_loop(self):
        ...
        while self.running:
            try:
                ...
                frame = self.frame_buffer.get(timeout=0.05)  # Reduced from 0.1 to 0.05 for faster processing
                ...
                if self.total_frames_processed % 100 == 0:  # Reduced from 20 to 100 frames
                    logger.debug(f"Performance stats - FPS: {processing_fps:.1f}, Avg processing time: {avg_processing_time*1000:.1f}ms")
                    logger.debug(f"Processing stats - FPS: {processing_fps:.1f}, Queue: {current_sequence_count}/{self.sequence_length}, Total frames: {self.total_frames_processed}")
    ...
    def process_frame(self, frame: np.ndarray) -> bool:
        ...
        try:
            ...
            if self.frame_buffer.qsize() % 10 == 0:  # Log every 10 frames
                ...
                with self.lock:
                    current_sequence_count = self.frames_collected
                logger.info(f"Queue status: {self.frame_buffer.qsize()}/{self.max_queue_size} frames (Sequence: {current_sequence_count}/{self.sequence_length})")
            return True
        except Exception as e:
    ...
    def get_sequence(self) -> Optional[np.ndarray]:
        ...
        with self.lock:
            ...
            # Log before clearing
            logger.info(f"[SEQUENCE] RETRIEVING SEQUENCE: {self.frames_collected} frames, clearing buffer for next cycle")
