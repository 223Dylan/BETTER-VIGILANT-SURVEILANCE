...
class WebSocketManager:
    ...
    async def connect_camera(self, camera_id: str, websocket: WebSocket):
        ...
        logger.info(f"[WEBSOCKET] Camera WebSocket connected for {camera_id}. Total connections: {len(self.camera_connections[camera_id])}")
    ...
    async def connect_prediction(self, camera_id: str, websocket: WebSocket):
        ...
        logger.info(f"[WEBSOCKET] Prediction WebSocket ADDED for {camera_id}. Total connections: {len(self.prediction_connections[camera_id])}")
        logger.info(f"[INFO] Current prediction connections: {list(self.prediction_connections.keys())}")
    ...
    async def broadcast_prediction(self, camera_id: str, prediction_data: dict):
        ...
        logger.info(f"[INFO] Available cameras with connections: {list(self.prediction_connections.keys())}")
        
        if camera_id not in self.prediction_connections:
            logger.warning(f"[WARNING] No prediction connections for camera {camera_id}")
            return
        ...
        for websocket in connections.copy():
            try:
                await websocket.send_json(message)
                sent_count += 1
                logger.info(f"[SUCCESS] Sent prediction to connection #{sent_count} for {camera_id}")
            except Exception as e:
        ...
        logger.info(f"[SENT] Successfully sent prediction to {sent_count} connections for {camera_id}")
    ...
    def get_connection_count(self, camera_id: str) -> dict:
        ...
        return {
            ...
            "prediction_connections": len(self.prediction_connections.get(camera_id, set()))
        }
