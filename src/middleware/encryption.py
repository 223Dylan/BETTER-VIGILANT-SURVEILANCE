...
class EncryptionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if not any(
            path.startswith(p.replace("{camera_id}", "")) for p in self.encrypted_paths
        ):
...
class RequestSigningMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        path = request.url.path
        if not any(
            path.startswith(p.replace("{camera_id}", "")) for p in self.signed_paths
        ):
            return await call_next(request)
        ...
        try:
            ...
            timestamp_time = time.mktime(
                time.strptime(timestamp, "%Y-%m-%dT%H:%M:%S.%f")
            )
